...
window.navigator.geolocation.getCurrentPosition(
  ...
  (e) => {
    loadMap([37.8896, 41.1327], "Varsayılan Konum");
  }
);
function loadMap(currentPosition, msg) {
  ...
  // Ekrana basılacak işratelerin listeleneceği bir katman oluştur
  ...
  // Haritaya tıklama olayı gerçekleşince
...
ui.cancelBtn.addEventListener("click", () => {
  // Aside a eklenen "add" classını kaldır
...
ui.form.addEventListener("submit", (e) => {
  ...
  e.preventDefault();
  // Form içerisindeki verilere eriştik
...
function renderMakers() {
  ...
  // Notlar içindeki her bir öğe için bir işaretci ekle
  notes.map((note) => {
    const icon = getIcon(note.status); // İkonu al

    // Eğer ikon geçerli değilse, hata mesajı göster ve devam etme
    if (!icon) {
      return; // İkon yoksa, bu notu haritaya ekleme
    }

    // Geçerli bir ikon varsa, marker'ı haritaya ekle
...
function deleteNote(id) {
  ...
  if (res) {
    // id si bilinen elemanı notes dizisinde kaldır
...
// * Arrow iconuna tıklayınca çalışacak fonksiyon
